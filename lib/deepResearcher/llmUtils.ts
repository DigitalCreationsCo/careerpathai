...
import { AIMessage, BaseMessage, ChatMessage, filterMessages, FunctionMessage, HumanMessage, SystemMessage, ToolMessage } from "@langchain/core/messages";
import { RunnableConfig } from "@langchain/core/runnables";

// Get API key for a specific model from environment or config.
export function getApiKeyForModel(modelName, config) {
    const shouldGetFromConfig = Boolean((process.env.GET_API_KEYS_FROM_CONFIG || "false").toLowerCase());
